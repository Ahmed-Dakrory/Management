...
@register.simple_tag
def get_total_row_by_data(category,day_to_generate):
    # print(day_to_generate)
    col = size
    row = category
    ...
    with connection.cursor() as cursorLast:
        try:
            ...
            sql_query = """
                ...
                left join size on item.size_id=size.id
                left join category on item.category_id=category.id
                where category.id="""+str(row.id)+""" and ( type_of_transaction_id="""+str(settings.ID_ADD_TYPE_OF_TRANSACTION)+"""  or type_of_transaction_id="""+str(settings.ID_RETURN_TYPE_OF_TRANSACTION)+""")
                and DATE_FORMAT(STR_TO_DATE(`transaction`.`created`, '%Y-%m-%d'), '%d-%m') = DATE_FORMAT(STR_TO_DATE('"""+str(day_to_generate.year)+"""-"""+str(day_to_generate.month)+"""-"""+str(day_to_generate.day)+"""','%Y-%m-%d'), '%d-%m')
        ...
        try:
            sql_query = """
                ...
                left join size on item.size_id=size.id
                left join category on item.category_id=category.id
                where category.id="""+str(row.id)+""" and ( type_of_transaction_id="""+str(settings.ID_OUT_TYPE_OF_TRANSACTION)+""")
                and DATE_FORMAT(STR_TO_DATE(`transaction`.`created`, '%Y-%m-%d'), '%d-%m') = DATE_FORMAT(STR_TO_DATE('"""+str(day_to_generate.year)+"""-"""+str(day_to_generate.month)+"""-"""+str(day_to_generate.day)+"""','%Y-%m-%d'), '%d-%m')
